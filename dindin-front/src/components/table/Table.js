...
function Table({ item, setModal, requireLines, mascaraMoeda }) {
    ...
    function deleteLine() {
        ...
        try {
            fetch(`${rota}/transactions/${item.id}`, {
                method: 'DELETE'
            });
            requireLines()
        } catch (error) {
    ...
    return (
        <div className="table-line">
            ...
            <h2 id="value" className={`valor-line ${type === "credit" ? "line-in" : "line-out"}`}>{type === 'credit' ? value : `-${value}`}</h2>
