import '../section-filter/filter.css';
import addTag from '../../assets/addTag.png';
import removeTag from '../../assets/removeTag.png';
...
function FilterTag({ func, nome, ativo, filtroSetings, setFiltroSetings }) {
    ...
    return (
        <button onClick={selectTag} className={`container-chip ${func(nome)}`}>
            <span className="tag-filter">{nome}</span>
            <img src={func(nome) === 'selected' ? removeTag : addTag} alt="tag" className="icon-filter" />
        </button>
    )
}

function Filter({ diasFiltro, categorias, filtroSetings, setFiltroSetings: setFiltroSettings }) {
    ...
    function aplicarFiltro(event) {
        event.preventDefault()
        const filtroAply = { ...filtroSetings, apply: true }
        setFiltroSettings(filtroAply)
    }
        ...
        <section className={`container-filters ${!filtroSetings.aberto && "hidden"}`}>
            <div className="dias filter-column">
                ...
                <div className="container-tags">
                    {
                        diasFiltro.map(item => <FilterTag func={handleClassDia} nome={item.nome} ativo={item.tipo} filtroSetings={filtroSetings} setFiltroSetings={setFiltroSettings} key={item.nome} />)
                    }
                </div>
            </div>
            <div className="categorias filter-column">
                ...
                <div className="container-tags">
                    {
                        categorias.map(item => <FilterTag func={handleClassTag} nome={item.nome} ativo={item.tipo} filtroSetings={filtroSetings} setFiltroSetings={setFiltroSettings} key={item.nome} />)
                    }
                </div>
            </div>
